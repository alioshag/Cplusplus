...
bool date::install(int initial_month, int initial_day, int initial_year)
{
	if (initial_month < january || initial_month > december) {
		cerr << "bad month "
			<< initial_month << "/" << initial_day << "/" << initial_year << "\n";
		return false;
	}

	if (initial_day < 1 || initial_day > length[initial_month]) {
		cerr << "bad day "
			<< initial_month << "/" << initial_day << "/" << initial_year << "\n";
		return false;
	}
