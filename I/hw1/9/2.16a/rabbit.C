...
//const char rabbit::c = 'r';
rabbit::rabbit(const terminal& initial_t, unsigned initial_x, unsigned initial_y)
        : t(&initial_t), x(initial_x), y(initial_y)//Initialization of data members rather than assign values to it bellow
{
	//t = &initial_t;
	//x = initial_x;
	//y = initial_y;
...
rabbit::~rabbit()
{
        t->beep();   //beep the terminal when animal is dying

        t->wait(1000);  //Freeze the animal
        
        //when rabbit died, replace rabbit with background char on the terminal
        if (t->get(x, y) == c) {   //compares rabbits position on terminal with its char
                t->put(x, y);      //if true, replace rabbit with background char on terminal
        }
}
